...
class Conv2d_Bezier(Conv2d):
    ...
    def _sample_parameters(self, λ: float) -> Tuple[torch.Tensor]:
        ...
        elif len(self.weight) == 4:
            w += (1 - λ) * (1 - λ) * (1 - λ) * self.weight[0]
            w += 3 * (1 - λ) * (1 - λ) * λ * self.weight[1]
            w += 3 * (1 - λ) * λ * λ * self.weight[2]
            w += λ * λ * λ * self.weight[3]
            if b is not None:
                b += (1 - λ) * (1 - λ) * (1 - λ) * self.bias[0]
                b += 3 * (1 - λ) * (1 - λ) * λ * self.bias[1]
                b += 3 * (1 - λ) * λ * λ * self.bias[2]
                b += λ * λ * λ * self.bias[3]
